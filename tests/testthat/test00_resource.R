...
{
    srv <- sub$get_resource_group(rgname)$get_kusto_cluster(srvname)
    tok1 <- srv$get_aad_token()
    expect_true(AzureRMR::is_azure_token(tok1))

    tok2 <- get_kusto_token(clustername=srvname, location="australiaeast", tenant=tenant)
    expect_true(AzureRMR::is_azure_token(tok2))

    srvuri <- paste0("https://", srvname, ".australiaeast.kusto.windows.net")
    tok3 <- get_kusto_token(srvuri, tenant=tenant)
    expect_true(AzureRMR::is_azure_token(tok3))

    expect_identical(tok1$hash(), tok2$hash())
    expect_identical(tok1$hash(), tok3$hash())
    ...
    expect_null(delete_kusto_token(hash=tok1$hash(), confirm=FALSE))
})
